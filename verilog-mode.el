...
(defconst verilog-mode-version "20200312"
...
Provides DOCSTRING PROPS in newer Emacs (23.1)."
  (condition-case nil
    ...
    (error (define-abbrev-table tablename definitions))))
...
Provides SYSTEM-FLAG in newer Emacs."
  (condition-case nil
    ...
    (error (define-abbrev table name expansion hook))))
...
(defcustom verilog-indent-level 4
  "Indentation of Verilog statements with respect to containing block."
  :group 'verilog-mode-indent
  :type 'integer)
(put 'verilog-indent-level 'safe-local-variable 'integerp)

(defcustom verilog-indent-level-module 4
...
(defcustom verilog-indent-level-declaration 4
...
(defcustom verilog-indent-level-behavioral 4
...
(defcustom verilog-library-extensions '(".v" ".va" ".sv" ".svh")
...
so there may be a large up front penalty for the first search."
  (let (pt)
    (while (and (not pt)
                (re-search-forward regexp bound noerror))
      (if (verilog-in-comment-or-string-p (match-beginning 0))
...
Buffer changes will be ignored.  See `verilog-in-comment-or-string-p'
and `verilog-scan'.")
...
(defun verilog-scan-region (beg end)
  "Parse between BEG and END for `verilog-in-comment-or-string-p'.
...
(defun verilog-in-comment-p (state)
  "Return true if in a `/*' or `//' comment."
  (nth 4 state))

(defun verilog-in-star-comment-p (state)
  "Return true if in a star comment."
  (and (nth 4 state)
       (not (nth 7 state))))

(defun verilog-in-slash-comment-p (state)
  "Return true if in a slash comment."
  (nth 7 state))

(defun verilog-in-string-p (state)
  "Return true if in a string."
  (nth 3 state))
...
(defun verilog-in-paren-p (&optional state)
  "Return true if in a parenthetical expression.
May cache result using `verilog-syntax-ppss'."
  (let ((state (or state (save-excursion (verilog-syntax-ppss)))))
    (> (nth 0 state) 0)))
...
(defun verilog-in-struct-p ()
  ...
  (interactive)
  (save-excursion
    (if (verilog-in-paren-p (verilog-syntax-ppss))
        (progn
...
(defun verilog-right-after-star-comment-p ()
  (and (not (bobp))
       (= (char-before) ?\/)
       (= (char-before (1- (point))) ?\*)))
...
to establish comment properties on all text."
  (let ((state (verilog-syntax-ppss pos)))
    (or (verilog-in-comment-p state)
        (verilog-in-string-p state))))

(defun verilog-insert (&rest stuff)
  "Insert STUFF arguments, tracking for `verilog-in-comment-or-string-p'.
...
(defun verilog-beg-of-statement-1 ()
  "Move backward to beginning of statement."
  (interactive)
  (if (verilog-in-comment-p (save-excursion (verilog-syntax-ppss)))
      (verilog-backward-syntactic-ws))
  (let ((pt (point)))
    (catch 'done
...
(defun verilog-backward-up-list (&optional arg)
...
(defun verilog-strip-comments ()
  ...
  (while (re-search-forward "//" nil t)
    (if (verilog-within-string)
      ...
      (if (verilog-in-star-comment-p (save-excursion (verilog-syntax-ppss)))
          (re-search-forward "\\*/" nil t)
        (let ((bpt (- (point) 2)))
          (end-of-line)
...
Return a list of two elements: (INDENT-TYPE INDENT-LEVEL)."
  (save-excursion
    (let* ((starting_position (point))
           ...
           (lim (save-excursion (verilog-re-search-backward verilog-beg-block-re nil t)))
           (structres nil)
           ...
           (type (catch 'nesting
                   ...
                   (if (verilog-in-star-comment-p state)
                   ...
                   (when verilog-indent-lists
                     (save-excursion
                       (back-to-indentation)
                       (cond
                        ((verilog-at-paren-close-p)
                         (throw 'nesting 'paren-close))
                        ((verilog-in-paren-p state)
                         (setq par 1)
                         (throw 'nesting 'block))
                        (t nil))))
    ...
    (let* ((type (verilog-calc-1))
      ...
      (if (and
           ...
           (not(or (verilog-in-coverage-p)
                   (verilog-in-struct-p)))
           (verilog-in-paren-p (verilog-syntax-ppss)))
          (setq depth 1)
        (cond
         ((eq type 'case)
           ...
           ((looking-at verilog-beg-block-re-ordered)
            (cond
             ...
             ((match-end 17)  ; *sigh* might be a clocking declaration
              (let ((here (point)))
                (cond ((verilog-in-paren-p (verilog-syntax-ppss))
                       t) ; this is a normal statement
                      ((save-excursion
                         (verilog-beg-of-statement)
...
(defun verilog-skip-forward-string ()
  (search-forward "\""))

(defun verilog-skip-backward-string ()
  (search-backward "\""))

(defun verilog-skip-forward-double-slash-comment ()
  (forward-line 1))

(defun verilog-skip-backward-double-slash-comment ()
  (search-backward "//")
  (skip-chars-backward "/"))

(defun verilog-skip-forward-star-comment ()
  (search-forward "*/"))

(defun verilog-skip-backward-star-comment ()
  (search-backward "/*"))
...
(defun verilog-skip-forward-comment-or-string ()
  ...
  (let ((state (save-excursion (verilog-syntax-ppss))))
    (cond
     ((verilog-in-string-p state)
      (verilog-skip-forward-string)
      t)
     ((verilog-in-slash-comment-p state)
      (verilog-skip-forward-double-slash-comment)
      t)
     ((verilog-in-star-comment-p state)
      (verilog-skip-forward-star-comment)
      t)
     (t
      nil))))
...
(defun verilog-skip-backward-comment-or-string ()
  ...
  (let ((state (save-excursion (verilog-syntax-ppss))))
    (cond
     ((verilog-in-string-p state)
      (verilog-skip-backward-string)
      t)
     ((verilog-in-slash-comment-p state)
      (verilog-skip-backward-double-slash-comment)
      t)
     ((verilog-in-star-comment-p state)
      (verilog-skip-backward-star-comment)
      t)
     (t
      nil))))
            ...
            (let ((state (save-excursion (verilog-syntax-ppss))))
              (cond
               ...
               ((verilog-in-star-comment-p state)
                ...
                (skip-chars-backward " \t\n\f")
                t)
               ((verilog-right-after-star-comment-p)
                (goto-char (- (point) 2)) ;; Move back into star comment
                t)
               ((and (not (bobp))
                     (= (char-before) ?\))
                     (= (char-before (1- (point))) ?\*)
  ...
  (let ((type (car indent-str))
    ...
    (cond
         ...
         ((or
          ...
          (let* ( fst
            ...
            (if (and (not verilog-indent-lists)
                     (verilog-in-paren-p (save-excursion (verilog-syntax-ppss))))
     ...
     (; handle inside parenthetical expressions
      (eq type 'cparenexp)
      (let* ((val (+ verilog-indent-level (verilog-current-indent-level))))
        (indent-line-to val)))
...
(defun verilog-indent-comment ()
  ...
  (let* ((stcol
          (cond
           ((verilog-in-star-comment-p (save-excursion (verilog-syntax-ppss)))
            (save-excursion
...
If QUIET is non-nil, do not print messages showing the progress of line-up."
  ...
  (unless (verilog-in-comment-or-string-p)
    (save-excursion
      (beginning-of-line)
      (when (and (looking-at verilog-assignment-operation-re)
                 (save-excursion
        ...
        (let* ((start (save-excursion ; BOL of the first line of the assignment block
                        ...
                        (let ((pt (point)))
                          (forward-line -1)
                          (beginning-of-line)
                          (while (and (looking-at verilog-assignment-operation-re)
                                      (save-excursion
                                        (goto-char (match-end 2))
                                        (not (verilog-in-paren-p)))
                                      (not (bobp)))
                            (setq pt (point))
                            (forward-line -1)
  ...
  (save-excursion
    (let ((end-mod-point (verilog-get-end-of-defun))
      ...
      (while (verilog-re-search-forward-quick "\\(/\\*AUTOINST\\((.*?)\\)?\\*/\\|\\.\\*\\)" end-mod-point t)
        (save-excursion
          (goto-char (match-beginning 0))
          (setq par-values (verilog-read-inst-param-value))
          (unless (verilog-in-comment-or-string-p)
      ...
      (when (and (not hit)
        ...
        (if (and (car (verilog-library-filenames
            ...
            (find-file (car (verilog-library-filenames
                             ...
                             (buffer-file-name))))
          (when warn
            (message "File `%s' isn't readable, use shift-mouse2 to paste in this field"
                     (match-string 1))))))))
